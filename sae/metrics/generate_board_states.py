...
def reconstruct_board_states_from_games(games_filepath: str, output_filepath: str = None):
    ...
    Args:
        ...
        output_filepath: Ruta donde guardar board_states_*.npy (opcional)
        
    Returns:
        board_states: Array de forma (n_games, n_moves, 8, 8)
    """
    ...
    print(f"Rango de valores: [{board_states.min()}, {board_states.max()}]")
    print(f"Valores únicos: {np.unique(board_states)}")
    ...
    if output_filepath is not None:
        ...
        np.save(output_path, board_states)
